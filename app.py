...
# Inyectamos CSS para imitar el diseño de Tkinter
st.markdown("""
    <style>
        ...
        /* Juntar los botones de la cuadrícula */
...
try:
    ...
    if ganador > 0:
        ...
        st.code(marcador)
        if st.button("Jugar de nuevo"): st.session_state.clear(); st.rerun()
        st.stop()
except:
    pass
...
# --- FASE 4: COMBATE ---
    ...
    if turno_actual == st.session_state.mi_id:
        st.error(f"🔥 ¡ES TU TURNO JUGADOR {st.session_state.mi_id}! Dispara en el Radar.")
    else:
        st.warning(f"⏳ Turno del Jugador {turno_actual}... Prepara tus defensas.")
        ...
        for x in range(tamano):
            ...
            for y in range(tamano):
                ...
                if (x, y) in st.session_state.mis_coordenadas:
                    ...
                    if valor == -1:
                        cols[y].button("⬜", key=f"def_{x}_{y}", disabled=True) # Alguien disparó al agua
                    else:
    ...
    with col_der:
        ...
        for x in range(tamano):
            ...
            for y in range(tamano):
                valor = matriz[x][y]
                if valor == -1:
                    cols[y].button("⬜", key=f"atk_{x}_{y}", disabled=True) # Disparo fallido (Agua)
                elif valor > 0:
                    cols[y].button("🟥", key=f"atk_{x}_{y}", disabled=True) # Barco Enemigo Hundido
                else:
                    # Casilla sin descubrir
                    if turno_actual == st.session_state.mi_id:
                        if cols[y].button("🎯", key=f"atk_{x}_{y}"):
                            res = servidor.disparar(st.session_state.mi_id, x, y)
                            if res > 0: st.toast(f"💥 ¡Impacto confirmado al Jugador {res}!", icon="🔥")
                            else: st.toast("💦 ¡Cayó en el agua!", icon="💧")
                            st.rerun()
                    else:
                        cols[y].button("🟦", key=f"atk_disabled_{x}_{y}", disabled=True)

    # Refrescar pantalla si no es mi turno
    if turno_actual != st.session_state.mi_id:
        time.sleep(2)
        st.rerun()
